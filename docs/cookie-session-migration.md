...
- Phase 1：
  ...
  - [x] **bootstrap 重构为 setSession 恢复原始会话（取代 custom-ticket）——真实账号 E2E 已通过。**
    ...
    - **真实账号 E2E（memory-only，云游君）已验证**：邮箱 OTP 登录 → 清空 localStorage+sessionStorage + reload → setSession 恢复 → **非匿名、token 不落 localStorage、无闪、无控制台错误**；`/wallet` 余额 1006、会员有效（均通过 `account-api`）全部正常。**证明恢复的会话对 callFunction 鉴权完整。**
