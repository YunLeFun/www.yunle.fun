...
> 状态：实施中（Phase 0 起）。本文是迁移的唯一权威设计参考，按阶段推进，每阶段独立可发布。
...
| **服务端铸票原语已在用** | [sso-ticket/index.js](../cloudfunctions/sso-ticket/index.js) `auth().createTicket(uid)`（RS256 JWT，票据 10 分钟过期） | 直接复用做「cookie → 内存登录」 |
| 已有内部服务 token 范式 | `ACCOUNT_API_INTERNAL_TOKEN`（[.env.example:31](../.env.example)）；account-api `deductCoinForUser` 等 | 铸票端点照搬此鉴权范式 |
...
- **第二层（临时、功能）= CloudBase SDK 内存 token**：`persistence:'none'`。页面启动时拿 cookie 向服务端换一张 `createTicket` 短票，客户端 `signInWithCustomTicket(ticket)` 建立内存登录态。**SDK 照常直连 31 处数据访问，零改动。**

```
浏览器                         Nuxt SSR 服务端(www)            CloudBase(api)
─ httpOnly cookie(持久真值)  ─ 读 cookie → 正确首屏          ─ createTicket(uid)[内部token]
─ CloudBase SDK(内存 token)  ─ /session: 设cookie · 换票      ─ account-api 等 6 函数(uid运行时注入·不变)
─ 数据直连 ×31(不变)         ─ OAuth code 交换搬这           ─ DB / storage(不变)

① 登录 → set-cookie    ② cookie→票→内存登录    ③ 数据调用: SDK→CloudBase(内存token·不变)
```
...
- **换票登录 = `signInWithCustomTicket(getTicketFn)`**，参数是一个**返回 `Promise<string>` 的 getter**，不是票据字符串。客户端启动：`auth.signInWithCustomTicket(() => $fetch('/api/session/bootstrap').then(r => r.ticket))`。
- **persistence 实况（已实测，重要）**：`@cloudbase/js-sdk@3.3.13` 的 `persistence` **只认 `'local'`**——浏览器实测 `'session'` 与 `'none'` 都回落到 localStorage（`credentials_<env>` 始终写 localStorage）。**故无法靠 `persistence` 把 refresh token 移出 JS 可读区，XSS 硬化这一安全目标本方案当前并未达成。** 已把 useCloudbase 的 persistence 改回 `'local'`。
  - **自定义内存 storage 适配器：已验证可行（关键结论：不必上纯 BFF）。** `AuthOptions.storage?: SimpleStorage`（6 方法）是注入点；注入 Map 内存实现后实测 `credentials_<env>` **不再落 localStorage**，会话由 cookie→ticket 重建、鉴权数据访问（账户余额等）正常。实现见 [useCloudbase.ts](../app/composables/useCloudbase.ts) `createMemoryStorage`。
  - **但内存化引入「启动竞态」**：内存每次 load 都为空，bootstrap（cookie→ticket）完成前提前触发的鉴权调用会 403（实测 `apps` 集合直读、`加载个人数据失败`）。**同一竞态也存在于基础 cookieSession 路径的「新标签/清存储」恢复场景**（普通 reload 因 localStorage 有 token 不受影响，实测干净）。
...
- **EdgeOne 部署（平台侧动作，需账号）——⚠️ 实测尚未生效**：线上 `POST /api/session/*` 仍 404（`server: edgeone-pages`），即当前部署没有 serve SSR 运行时。仍需：
  ...
  - EdgeOne env 配 `NUXT_SESSION_PASSWORD`（≥32 字符）+ `NUXT_SSO_TICKET_*`，**否则 build/runtime 失败**。
- **风险**：EdgeOne SSR 冷启动、`/u` SSR 同时真正生效。

### Phase 1 · 会话层 + 换票（安全核心，token 出 localStorage）
- **改动**：
  - `server/api/session/login.post.ts`：收前端 CloudBase 登录后的 access_token → 服务端向 `/auth/v1/user/me` 校验并取 uid（杜绝伪造）→ `setUserSession({ user:{ uid } })`。
  - `server/api/session/bootstrap.post.ts`：`requireUserSession` 拿 uid → 调 CloudBase 铸票端点（内部 token）→ 返回 ticket。
  - `server/api/session/logout.post.ts`：`clearUserSession`。
  - CloudBase：给 sso-ticket 加内部 token 鉴权的 `mintForUser(uid)` 路径（私钥仍只在 CloudBase）。
  - 客户端 [useCloudbase.ts:31](../app/composables/useCloudbase.ts) `persistence:'local'` → `'session'`（见上「WebV3 对齐」：web SDK 无确认的内存模式，`'none'` 待实测可则优先）；启动流程：加载 → `signInWithCustomTicket(() => $fetch('/api/session/bootstrap').then(r => r.ticket))` → 数据直连照常。
- **验收**：refresh_token 不再落 localStorage（退至 sessionStorage 或内存）；刷新后经 cookie+bootstrap 恢复登录态；数据访问正常。

### Phase 2 · 改造登录入口去种 cookie
- OTP / 密码：CloudBase 登录成功后把会话交给 `/session/login` 种 cookie。
...
- **CSRF**：cookie 会话特有。对 `/session/*` 等靠 cookie 的端点加 double-submit token 或 SameSite + 自定义 header 校验；bootstrap 限速、票据一次性。

## 5. CloudBase 铸票端点契约（`mintForUser`）

新增（或在 sso-ticket 内分支）一个内部 token 鉴权的服务端调用：
...
| `NUXT_SSO_TICKET_MINT_URL` | 服务端调铸票端点地址（如 `https://api.yunle.fun/sso-ticket`） | Nuxt 服务端 |
| `NUXT_SSO_TICKET_INTERNAL_TOKEN` | 调铸票端点的内部 token | Nuxt 服务端（私有，勿入 public） |
| `SSO_TICKET_INTERNAL_TOKEN` | sso-ticket 校验内部调用 | CloudBase 函数 env |
...
- 安全 review 在 Phase 5 收口：CSRF、cookie flags、票据一次性与限速、私钥边界。
...
- Phase 0：
  ...
  - [ ] **EdgeOne 真正 serve SSR 运行时（实测未生效）**：repo `build` 已是 `nuxt build`，但线上 `POST /api/session/*` 仍 404（`server: edgeone-pages`）→ 当前部署无服务端运行时。需构建命令 `pnpm build` + **`@edgeone/nuxt-pages` 适配或原生 SSR（真正托管 `.output/server`）** + EdgeOne env（`NUXT_SESSION_PASSWORD`/`NUXT_SSO_TICKET_*`）。控制台需账号；适配器可在 repo 内加。
- Phase 1：
  ...
  - [x] **真实账号 E2E（本地 dev + flag on）**：邮箱 OTP 登录→httpOnly cookie 种上；`/api/session/bootstrap` 200 + 真实 ticket；**清空 localStorage+sessionStorage 后 reload，仍留在 /wallet、头像恢复、无登录闪、无控制台错误**——会话纯由 cookie 恢复 ✅
  ...
  - [ ] 纵深防御（Phase 5）：严格 CSP + 短 token TTL + `/session/*` CSRF
  - [ ] **prod 激活**：EdgeOne env 置 `NUXT_PUBLIC_COOKIE_SESSION=true` + `NUXT_SSO_TICKET_MINT_URL`/`NUXT_SSO_TICKET_INTERNAL_TOKEN`/`NUXT_SESSION_PASSWORD`
