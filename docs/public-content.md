...
iOS 开放后，在现有 `url` 字段填写真实的 App Store HTTPS 商品页地址。
官网在电脑和手机端统一展示「前往 App Store」，在新标签页打开商品页，由 Apple 处理设备适配和安装入口。
不自建二维码，不增加 Mac 专属配置。若同一商品支持 iOS 和 Mac，共用该链接；未开放时继续显示「暂未开放」。
