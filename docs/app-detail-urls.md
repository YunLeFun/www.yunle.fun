...
生产发布和迁移已于 2026-09-06 完成，验收依据见下方记录。
...
- EdgeOne 生产部署成功：官网 `0ce7f2b`、应用平台 `22b73a5`、管理后台 `f0077fb`；相关 GitHub CI 全部通过。
- `account-api` 已更新并恢复 Active；下载的线上 `tips.js` 与提交一致，函数配置和环境变量校验值保持一致。
- 生产环境 `yunlefun-8g7ybcxc7345c490` 已备份并分阶段迁移：29 个应用、27 个市场登记、56 个身份登记；另有 1 个迁移状态记录。
- `owner_slug_unique` 联合唯一索引已建立，旧全局 slug / nameKey 唯一索引已移除，`migration_v2.ready=true`，应用管理写入恢复正常鉴权。
- 真实 CloudBase 事务验证通过：不同所有者可登记相同 slug；并发抢占同一市场短名仅一方成功。临时测试登记已清理，未创建用户或金融流水。
- 官网公共短名接口、所属空间接口与页面返回正常；小写所有者查询有效，不存在的所有者返回 404；公共接口返回 `Cache-Control: no-store`。
- 认证后的真实用户投稿、审批及投币未在生产执行；这些流程由本地测试覆盖，避免为验收修改真实业务记录。

运维使用已登录的 `tcb secrets get --json` 会话在内存中注入临时凭据（SDK 使用 sessionToken，manager 使用 token），无需把密钥写进仓库。
