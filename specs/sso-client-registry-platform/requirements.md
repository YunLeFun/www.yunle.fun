...
4. 通过 CI、部署 smoke 或独立受控任务读取已发布快照并执行 shadow compare，授权运行时不读取 Registry 管理数据库。
...
- **授权运行时**：只消费编译期静态 Registry，不读取管理数据库或审批状态。
...
### R6. P1 shadow compare

P1 中，静态 Registry 必须继续决定所有授权结果。

受保护 CI、部署 smoke 或独立受控监控任务必须：

- 通过私有管理函数读取活动平台快照
- 校验 Schema、contentHash、signature、environment 和 issuer
- 将平台快照与静态 Registry 的规范化内容进行比对
- 对一致、漂移、不可用、签名错误和版本回退输出结构化安全事件
- 在任何平台快照异常时阻止发布或告警，不改变当前静态授权结果

`sso-ticket` 与 `desktop-auth` 不得为该比较打包数据库 adapter、增加环境开关或在请求路径上等待远程读取。
...
6. 当 CI、部署 smoke 或独立监控执行 compare 时，系统应通过私有管理面取得并验证活动快照，授权请求不得参与该流程。
7. 当平台快照与静态 Registry 完全一致时，compare 应成功且不改变授权响应。
8. 当平台快照与静态 Registry 不一致时，compare 应记录包含环境、版本和哈希的漂移并阻止发布或告警。
9. 当数据库不可用、快照 Schema 无效、签名无效或版本回退时，compare 应失败；当前静态授权应保持不变。
...
- `sso-ticket` 与 `desktop-auth` 只携带 generated JSON；CI/部署 smoke 可显式执行签名 compare。
- 全量 lint、typecheck、test、build 和 CloudBase 代码审查通过。
- 生产验证覆盖快照加载、无漂移、数据库不可用和签名失败时阻止发布或告警。
