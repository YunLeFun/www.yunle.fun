 ...
 * 「用户会话就绪」时运行 fn。
 *
 * user.value 存在即代表会话已就绪——双层会话下 CloudBase 会话由 bootstrap（cookie→ticket）建立后才会
 * 置 user，故以 user 为准能保证 fn 里的鉴权请求不会在 token 就绪前触发（避免 bootstrap 窗口内 403）。
 * - 已就绪：立即运行
 * - 未就绪（如 cookie→ticket 恢复窗口、memory-only 每次加载）：待 user 首次出现后运行一次
 *
...
export function onUserSession(fn: () => void) {
  ...
  // 公开路由不经全局中间件恢复登录态：若会话尚未校验，主动触发一次（从 cookie→ticket 恢复）
  if (!authReady.value)
    checkAuthStatus()
  if (user.value) {
    fn()
