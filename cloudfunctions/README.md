...
| `ACCOUNT_API_INTERNAL_TOKEN`       | 是   | 读取账号状态与会员事实的内部服务令牌，**须与 `account-api` 配同一值**                                                           |
| `AUTH_ISSUER_ENVIRONMENT`          | 是   | `production` 或 `development`；Web SSO 与设备授权共用的隔离 issuer profile                                                      |
| `DESKTOP_AUTH_SIGNING_KID`         | 是   | 当前 active entitlement 签发公钥 `kid`                                                                                          |
| `DESKTOP_AUTH_CANONICAL_URL`       | 是   | DPoP `htu` 的精确 HTTPS 地址，例如 `https://api.yunle.fun/desktop-auth`                                                         |
...
node scripts/deploy-function.mjs appstore-notify
...
CloudBase 默认云存储的公开文件兼容区与旧通用用户存储的实际桶配置需要分别核验；
当前 `user-storage-api` 的旧 `PRIVATE_COS_BUCKET` 路由保留不变，Drive 新素材通过
`ASSET_PRIVATE_COS_BUCKET=yunlefun-private-1325586649` 单独进入私有 COS。
`user-storage-api` 使用 SCF 运行角色临时凭证签发短期 URL，
