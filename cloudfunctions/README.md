...
| `ADVJS_AI_RUNTIME_ACCOUNT_API_TOKEN`          | ADV.JS Runtime 与 Admin 调用 AI 点数私有 action 的专用凭据；不得复用通用 account-api、旧 ai-gateway、Runtime 管理或测试身份 token。                                                                                             | 使用独立的 32～512 字节高熵随机值。          |
