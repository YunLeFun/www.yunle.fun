...
async function getTencentEmailStatus(client, { messageId, submittedAt }) {
  ...
  const resultRows = Array.isArray(result?.EmailStatusList) ? result.EmailStatusList : []
  return normalizeEmailStatus(resultRows.find(row => row?.MessageId === messageId) || resultRows[0])
}
