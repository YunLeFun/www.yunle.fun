...
describe('registry protected workflows', () => {
  it('accepts only a release intent id and stops after opening a generated-only PR', () => {
    ...
    expect(source).toContain('tcb --config-file cloudbaserc.ci.json login --cloudbase-api-key "$CLOUDBASE_API_KEY" -e "$CLOUDBASE_ENV_ID"')
    expect(source.indexOf('tcb --config-file cloudbaserc.ci.json login --cloudbase-api-key')).toBeLessThan(source.indexOf('record-ci'))
  ...
  it('verifies release files before entering environment-scoped deployment jobs', () => {
    ...
    expect(source).toMatch(/CLOUDBASE_API_KEY: \$\{\{ secrets\.CLOUDBASE_API_KEY \}\}/)
    expect(source).toMatch(/CLOUDBASE_ENV_ID: \$\{\{ vars\.CLOUDBASE_ENV_ID \}\}/)
    expect(source.match(/tcb --config-file cloudbaserc\.ci\.json login --cloudbase-api-key/g)).toHaveLength(2)
