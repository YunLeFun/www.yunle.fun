...
describe('registry protected workflows', () => {
  it('accepts only a release intent id and stops after opening a generated-only PR', () => {
    ...
    expect(source).toContain('node scripts/normalize-github-app-private-key.mjs')
    expect(source).toContain('private-key: $' + '{{ steps.app-private-key.outputs.private-key }}')
    ...
    expect(source).toContain('--status ci_failed')
    expect(source).toContain('--status superseded')
    expect(source.indexOf('--status ci_failed')).toBeLessThan(source.lastIndexOf('--status superseded'))
  ...
  it('finalizes a successful CI run from trusted main without executing pull request code', () => {
    ...
    expect(source).toContain('--status ci_failed')
    expect(source).toContain('--status superseded')
    expect(source.indexOf('--status ci_failed')).toBeLessThan(source.indexOf('--status superseded'))
